...
ipa_name="yijian-adhoc.ipa"
ipa_build_path="${project_dir_path}/ipa-build"
archive_path="${project_dir_path}/ipa-build/yijian-adhoc.xcarchive"
output_directory="${project_dir_path}/ipa-build/"

# 先删除本地旧的ipa
rm "${ipa_build_path}/${ipa_name}"

fastlane gym --workspace ${workspace_path_name} --scheme 'AtFirstSight-Adhoc' --output_name "yijian-adhoc" --configuration Adhoc --export_method "ad-hoc" --buildlog_path ${ipa_build_path} --archive_path ${archive_path} --output_directory ${ipa_build_path} --clean
...
# 打包成功则上传ipa
if [ -f "${ipa_build_path}/${ipa_name}" ]; then

   # sftp 上传步骤
   local_file_path="${ipa_build_path}/${ipa_name}"
   target_name='yijian-adhoc'
   display_image='yijian'
   user_name='ios'
   password='ios12345678'
   remote_path='http://10.0.1.112:8081/artifactory/iOS_afs_adhoc'
   ...
   #plist_remote_https_path="/hd01/webapp/adhoc.skytech.cc/htdoc/jfrog_plist_file/yijian_adhoc"
